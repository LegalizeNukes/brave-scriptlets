...
(function(){
    "use strict";
    // API endpoint, cache/timing limits, site mode, selectors, and shared CSS.
    const API_URL="https://returnyoutubedislikeapi.com/votes?videoId=", CACHE_LIMIT=100, UI_WAIT_LIMIT=6e3, IS_MOBILE_SITE=location.hostname==="m.youtube.com", formatter=new Intl.NumberFormat("en", {
        notation:"compact", maximumFractionDigits:1
    }), DESKTOP_BUTTON_SELECTORS=["dislike-button-view-model button",
    ...
    'button[aria-label*="Dislike" i]'], MOBILE_BUTTON_SELECTORS=["ytm-segmented-like-dislike-button-renderer dislike-button-view-model button",
    ...
    "#segmented-dislike-button button",
    'button[aria-label*="Dislike" i]'], MOBILE_LIKE_BUTTON_SELECTORS=["ytm-segmented-like-dislike-button-renderer like-button-view-model button",
    ...
    'ytm-slim-video-action-bar-renderer button[aria-label^="Unlike" i]'], LIKE_TEXT_SELECTORS=['like-button-view-model button span[role="text"]',
    "like-button-view-model button span",
    "#segmented-like-button button span",
    "ytm-like-button-renderer #text",
    'ytm-segmented-like-dislike-button-renderer button[aria-label*="Like" i] span'], SHORTS_ROOT_SELECTORS=['ytd-reel-video-renderer[is-active-reel="true"]',
    'ytd-reel-video-renderer[tab-identifier="shorts"]',
    "ytm-reel-player-overlay-renderer",
    "ytd-reel-video-renderer"], SHORTS_COUNT_SELECTORS=["#button-bar > reel-action-bar-view-model > dislike-button-view-model > toggle-button-view-model > button-view-model > label > div > span",
    'reel-action-bar-view-model dislike-button-view-model label div span[role="text"]',
    'dislike-button-view-model label span[role="text"]'], BASE_STYLE=".return-youtube-dislike-button,.return-youtube-like-button{display:inline-flex!important;align-items:center!important;column-gap:8px!important;flex:0 0 auto!important;overflow:visible!important;width:auto!important;min-width:auto!important;margin-inline-end:12px!important}.return-youtube-dislike-count,.return-youtube-like-count{flex:0 0 auto;margin-left:0!important;white-space:nowrap}";
    // Current video/request/update/observer/presentation state.
    let currentVideoId="", currentDislikes=null, currentLikes=null, requestController=null, requestVideoId="", updateTimer=0, updateDueAt=Infinity, observer=null, observerScope=null, observerMode="", observerExpiry=0, styleElement=null, desktopCss="", desktopRoot=null;
    // Bounded LRU cache for fetched vote data.
    const cache=new Map;
    // Format like/dislike totals using compact English notation.
    function formatCount(v){
        return formatter.format(v)
    }
    // Detect whether the current route is a YouTube Shorts page.
    function isShortsPage(){
        return location.pathname.startsWith("/shorts/")
    }
    // Resolve the current video ID from the route or page metadata.
    function getVideoId(){
    ...
    // Read an item from the bounded cache and refresh its LRU position.
    ...
    // Insert/update an item in the bounded LRU cache.
    function putCached(v, d){
        cache.has(v)&&cache.delete(v);
        cache.set(v, d);
        cache.size>CACHE_LIMIT&&cache.delete(cache.keys().next().value)
    }
    // Create the script's shared style element if it does not already exist.
    ...
    // Refresh the shared base styles plus the current desktop-specific CSS.
    function refreshStyles(){
        styleElement&&(styleElement.textContent=BASE_STYLE+"\n"+desktopCss)
    }
    // Remove desktop-specific state and generated CSS.
    ...
    // Remove injected mobile counts and marker classes.
    function clearInjectedCounts(){
        document.querySelectorAll(".return-youtube-dislike-count,.return-youtube-like-count").forEach(e=>e.remove());
        document.querySelectorAll(".return-youtube-dislike-button,.return-youtube-like-button").forEach(e=>e.classList.remove("return-youtube-dislike-button", "return-youtube-like-button"))
    }
    // Clear all UI changes made by this script.
    function clearPresentation(){
        clearDesktop();
        clearInjectedCounts()
    }
    // Abort the currently active API request, if any.
    ...
    // Disconnect the active UI observer and clear its expiry timer.
    ...
    // Reset request, observer, presentation, and video state during navigation.
    ...
    // Debounce page updates while preserving the earliest pending update time.
    ...
    // Fetch dislike data, and mobile like data, from the Return YouTube Dislike API.
    async function fetchDislikes(v){
        ...
        try{
            const r=await fetch(API_URL+encodeURIComponent(v), {
                signal:a.signal, credentials:"omit"
            });
            if(!r.ok)return null;
            const j=await r.json(), d=Number(j&&j.dislikes);
            if(!Number.isFinite(d)||d<0)return null;
            if(!IS_MOBILE_SITE){
                putCached(v, d);
                return d
            }
            const l=Number(j&&j.likes), data={
    ...
    // Find the desktop watch root corresponding to the current video.
    function findDesktopRoot(v){
        const roots=document.querySelectorAll("ytd-watch-flexy");
        let first=null, empty=null;
    ...
    // Render the dislike count on desktop using generated ::after CSS.
    function renderDesktop(d, r){
        const b=findFirst(r, DESKTOP_BUTTON_SELECTORS);
        if(!b)return null;
        clearInjectedCounts();
        if(desktopRoot&&desktopRoot!==r)desktopRoot.removeAttribute("data-return-youtube-dislike");
        desktopRoot=r;
        ...
        refreshStyles();
        return{
            mode:"desktop", scope:r
        }
    }
    // Create or update a mobile count span while matching nearby text styling.
    function ensureMobileCount(b, type, value, v, reference){
        const countClass=`return-youtube-${type}-count`, buttonClass=`return-youtube-${type}-button`;
    ...
    // Render dislike and, when needed, like counts on the mobile watch page.
    function renderMobile(d, l, v){
        const r=document.querySelector("ytm-watch")||document, b=findFirst(r, MOBILE_BUTTON_SELECTORS), likeButton=findFirst(r, MOBILE_LIKE_BUTTON_SELECTORS);
        if(!b)return null;
        clearDesktop();
        ensureStyleElement();
        const dislikeCount=ensureMobileCount(b, "dislike", d, v, findFirst(r, LIKE_TEXT_SELECTORS));
        const injectedLikeCount=likeButton&&likeButton.querySelector(".return-youtube-like-count"), nativeLikeCount=likeButton&&[...likeButton.querySelectorAll("span")].some(e=>!e.classList.contains("return-youtube-like-count")&&/\d/.test(e.textContent));
        l!==null&&likeButton&&(injectedLikeCount||!nativeLikeCount)&&ensureMobileCount(likeButton, "like", l, v, dislikeCount);
        return{
            mode:"mobile", scope:b.closest("ytm-watch")||b.closest("ytm-slim-video-action-bar-renderer")||b.closest("ytm-segmented-like-dislike-button-renderer")||b.parentElement||document.body
        }
    }
    // Replace the Shorts dislike-count text for the active reel.
    function renderShorts(d){
        clearDesktop();
        clearInjectedCounts();
        const r=findFirst(document, SHORTS_ROOT_SELECTORS), c=findFirst(r||document, SHORTS_COUNT_SELECTORS);
        if(!c)return null;
        const t=formatCount(d);
        c.textContent.trim()!==t&&(c.textContent=t);
        return{
            mode:"shorts", scope:r||c.closest("reel-action-bar-view-model")||document.body
        }
    }
    // Watch the rendered control area and schedule repairs if YouTube mutates it.
    function watchRenderedUi(x){
        ...
        observerScope=s;
        observerMode=m;
        m==="desktop"?observer.observe(s, {
            attributes:true, attributeFilter:["video-id"]
        }):observer.observe(s, {
            childList:true, subtree:true, characterData:m==="shorts"
        })
    }
    // Temporarily observe the whole document while waiting for target controls to appear.
    function waitForUi(){
    ...
    // Render the currently cached counts using the appropriate desktop/mobile/Shorts path.
    function renderCurrent(){
        if(!currentVideoId||currentDislikes===null||getVideoId()!==currentVideoId)return;
        let r;
        if(isShortsPage())r=renderShorts(currentDislikes);
        else{
            const d=findDesktopRoot(currentVideoId);
            r=d?renderDesktop(currentDislikes, d):renderMobile(currentDislikes, currentLikes, currentVideoId)
        }
        r?watchRenderedUi(r):waitForUi()
    }
    // Resolve navigation state, fetch missing data, and render the current video.
    async function updatePage(){
        ...
        else currentDislikes=data;
        renderCurrent()
    }
    // Reset script state as YouTube begins a navigation.
    function onNavigationStart(){
        resetForNavigation()
    }
    // Schedule a near-immediate update after YouTube finishes navigating.
    function onNavigationFinish(){
        scheduleUpdate(80)
    }
    // Reset and refresh after browser history navigation.
    function onHistoryNavigation(){
        resetForNavigation();
        scheduleUpdate(80)
    }
    // Navigation/media lifecycle listeners that keep the injected UI synchronized with YouTube.
