...
// @match        https://*.washingtonexaminer.com/*
...
// Merged from the newer ORIGINAL logic while preserving UPDATE-specific Brave compatibility behavior.
...
function removeDOMElement(...elements) {
  for (let element of elements) if (element) element.remove();
}

function hideDOMElement(...elements) {
  for (let element of elements) if (element) element.style = "display:none !important;";
}
...
function waitDOMElement(selector, tagName = "", callback, multiple = false) {
  new window.MutationObserver(function(mutations) {
    for (let mutation of mutations) for (let node of mutation.addedNodes) if (!tagName || node.tagName === tagName) if (node.matches(selector)) {
      callback(node);
...
function waitDOMAttribute(selector, tagName = "", attributeName = "", callback, multiple = false) {
  ...
  new window.MutationObserver(function(mutations) {
    for (let mutation of mutations) if (mutation.target.attributes[attributeName]) {
...
function header_nofix(header, cond_sel = "", msg = "", url = "") {
  // UPDATE compatibility: suppress BPC fallback/status notices.
  return false;
}

function clearPaywall(paywall, paywall_action) {
  if (paywall) if (!paywall_action) removeDOMElement(...paywall); else for (let elem of paywall) if (paywall_action.rm_class) elem.classList.remove(paywall_action.rm_class); else if (paywall_action.rm_attrib) elem.removeAttribute(paywall_action.rm_attrib);
}
...
function getArticleSrc(url, url_src, proxy, base64, selector, text_fail = "", selector_source = selector, selector_archive = selector) {
  ...
  GM.xmlHttpRequest({
    ...
    onload: function(response) {
      ...
      let recursive;
      if (url.startsWith("https://archive.")) if (url_fetch.includes("/https")) if (html.includes('<div class="TEXT-BLOCK"')) {
      ...
      if (!recursive) replaceDomElementExtSrc(url, url_src, html, proxy, base64, selector, text_fail, selector_source, selector_archive);
    }
  });
}
  ...
  GM.xmlHttpRequest({
    ...
    onload: function(response) {
      ...
      if (json_key) try {
        ...
        if (json) html = getNestedKeys(json, json_key);
      } catch (err) {
        void 0;
      }
...
function replaceDomElementExt(url, proxy, base64, selector, text_fail = "", selector_source = selector, selector_archive = selector) {
  ...
  if (!article) {
    if (archive_match && document.body) document.body.firstChild.before(archiveLink(url));
    return;
  }
  if (proxy) {
    if (!text_fail) if (archive_match) text_fail = "BPC > Try for full article text (no need to report issue for external site):\r\n"; else if (!matchUrlDomain(window.location.hostname, url)) text_fail = "BPC > failed to load from external site:\r\n";
    ...
    }); else replaceTextFail(url, article, proxy, text_fail);
  }).catch(function(err) {
...
function getSelectorLevel(selector) {
  if (selector.replace(/,\s+/g, ",").match(/[>\s]+/) && !selector.includes(":has(>")) selector = selector.replace(/,\s+/g, ",").split(",").map(x => x.match(/[>\s]+/) ? x + ", " + x.split(/[>\s]+/).pop() : x).join(", ");
  return selector;
}
...
function replaceDomElementExtSrc(url, url_src, html, proxy, base64, selector, text_fail = "", selector_source = selector, selector_archive = selector) {
  ...
  if (html) {
    ...
    window.setTimeout(function() {
      ...
      if (selector_level) selector_source = getSelectorLevel(selector_source);
      ...
      if (article_new) {
        if (article && article.parentNode) {
          if (url.startsWith("https://archive.")) {
            ...
            if (arch_dom) {
              if (arch_dom.firstChild) arch_dom = arch_dom.firstChild;
          ...
          window.setTimeout(function() {
            if (article.parentNode) {
              ...
              if (func_post) func_post();
            }
          }, 200);
        }
      } else replaceTextFail(url, article_link, proxy, text_fail.replace(":", no_content_msg));
    }, 200);
  } else {
    ...
    if (false && !url_src && url.includes("/https://www.thetimes.com/")) {
      ...
      if (article_link) article_link.before(externalLink([ "clearthis.page" ], "https://clearthis.page/?u={url}", encodeURIComponent(url_orig), "BPC > Try for full article text:"));
    }
  }
}

function replaceTextFail(url, article, proxy, text_fail) {
  // UPDATE compatibility: suppress failed-fetch notices and helper links.
  return false;
}
...
function amp_iframes_replace(weblink = false, source = "") {
  ...
  let par, elem;
  for (let amp_iframe of amp_iframes) if (!weblink) {
    if (amp_iframe.offsetHeight > 10) {
      ...
      if (amp_iframe.getAttribute("sandbox")) elem.sandbox = amp_iframe.getAttribute("sandbox");
...
function amp_redirect_not_loop(amphtml) {
  if (!check_loop()) window.location.href = amphtml.href; else {
...
function amp_unhide_subscr_section(amp_ads_sel = "", replace_iframes = true, amp_iframe_link = false, source = "") {
  ...
  if (amp_ads_sel) hideDOMStyle(amp_ads_sel, 5);
  if (replace_iframes) amp_iframes_replace(amp_iframe_link, source);
}
...
function amp_unhide_access_hide(amp_access = "", amp_access_not = "", amp_ads_sel = "", replace_iframes = true, amp_iframe_link = false, source = "") {
  ...
  if (amp_ads_sel) hideDOMStyle(amp_ads_sel, 6);
  if (replace_iframes) amp_iframes_replace(amp_iframe_link, source);
}
...
function ampToHtml() {
  window.setTimeout(function() {
    ...
    if (canonical) window.location.href = canonical.href;
  }, 1e3);
}
...
function refreshCurrentTab(not_loop = true, not_loop_msg = true) {
  if (!not_loop || !check_loop(5e3)) window.setTimeout(function() {
    window.location.reload(true);
  }, 500); else if (not_loop_msg) {
    let header = document.body && document.body.firstChild || document.documentElement;
...
function archiveLink(url, text_fail = "") {
  // UPDATE compatibility: do not inject archive helper links into pages.
  return document.createTextNode("");
}

function archiveLink_renew(url, text_fail = "") {
  // UPDATE compatibility: do not inject archive-renewal links.
  return document.createTextNode("");
}

function googleSearchToolLink(url, text_fail = "") {
  // UPDATE compatibility: do not inject external search-tool links.
  return document.createTextNode("");
}

function freediumLink(url, text_fail = "") {
  // UPDATE compatibility: do not inject external mirror links.
  return document.createTextNode("");
}
...
function externalLink(domains, ext_url_templ, url, text_fail = "") {
  // UPDATE compatibility: external fallback UI is intentionally disabled.
  return document.createTextNode("");
}

function removeClassesByPrefix(el, prefix) {
  let el_classes = el.classList;
  for (let el_class of el_classes) if (el_class.startsWith(prefix)) el_classes.remove(el_class);
}
...
function setCookie(names, value, domain = "", path = "/", days = 0, localstorage_hold = false) {
  ...
  if (names instanceof RegExp) ck_names = matchCookies(names); else if (typeof names === "string") ck_names = [ names ];
  for (let ck_name of ck_names) document.cookie = ck_name + "=" + (value || "") + (domain ? "; domain=" + domain : "") + (path ? "; path=" + path : "") + "; max-age=" + max_age;
...
function getArticleJsonScript() {
  ...
  let json_script;
  for (let script of scripts) if (script.innerText.match(/"(articlebody|text)":/i)) {
    json_script = script;
    break;
  }
  return json_script;
...
function restorePugpigLink(node, art_link_sel = "") {
  ...
  if (art_link) art_link.onmousedown = x => window.location.href = art_link.href;
}
...
function getArticleQuintype() {
  ...
  if (json_script) try {
    ...
    if (slug && !decodeURIComponent(window.location.pathname).includes(slug)) refreshCurrentTab();
    ...
    for (let par of pars) {
      ...
      for (let elem of story_elements) {
        ...
        if ([ "text", "title" ].includes(elem.type) && elem.text) {
          ...
          if (elem.subtype === "tweet") if (elem.metadata && elem.metadata["tweet-url"]) {
        ...
        } else if (![ "widget" ].includes(elem.type)) void 0;
        if (par_elem) article_new.appendChild(par_elem);
      }
    }
    if (!article_new.hasChildNodes()) article_new = "";
  } catch (err) {
    void 0;
  }
  return article_new;
...
function mediafin_reset_auth(article = "") {
  mediafin_init_auth();
  mediafin_get_auth();
  if (article) header_nofix(article, "", "BPC > no data yet (refresh page)");
}
...
function mediafin_get_auth() {
  ...
  let ls_token;
  if (ls_date && ls_date === now_date) ls_token = localStorage.getItem("###_auth_token") || ""; else {
    let url_auth = "https://api.mediafin.be/oktaproxy/oauth/token";
    getExtFetch(url_auth, "", {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
      body: atob("")
    }, auth_mediafin);
      ...
      if (json && json.content && json.content.paragraphs) {
        ...
        if (embedded && embedded.images && embedded.images[0] && embedded.images[0].aspectRatios["16/9"]) {
          ...
          if (img_ratio && img_ratio.url) {
            let lead_img = makeFigure(img_ratio.url, (img_item.caption || "") + (img_item.credit ? " ©" + img_item.credit : ""), {
              style: "width: 100%;"
            }, {
              style: "margin: 10px;"
            });
        ...
        for (let par of pars) {
          ...
          if (par.type === "text") {
            ...
            if (par.urn && par.aspectRatio && embedded && embedded.images) {
              ...
              if (img_item) {
                ...
                if (img_ratio && img_ratio.url) elem = makeFigure(img_ratio.url, (img_item.caption || "") + (img_item.credit ? " ©" + img_item.credit : ""), {}, {
                  style: "margin: 10px;"
                });
              }
            }
            ...
            if (par.urn && embedded && embedded.relatedArticles) {
              ...
              if (articles) {
                ...
                for (let art of articles.articles) if (art.content && art.content.main && art.content.main.title && art.paths && art.paths[0] && art.paths[0].path) {
          ...
          } else if (![ "advertisement", "applicationHook", "citation", "html", "insetbox", "numbercitation", "tweet" ].includes(par.type)) void 0;
          if (elem) article.appendChild(elem);
        }
...
function filterObject(obj, filterFn, mapFn = function(val, key) {
  ...
  return Object.fromEntries(Object.entries(obj).filter(([key, val]) => filterFn(val, key)).map(([key, val]) => mapFn(val, key)));
}

function matchKeyJson(key, keys) {
  let match = false;
  if (typeof keys === "string") match = key === keys; else if (Array.isArray(keys)) match = keys.includes(key); else if (keys instanceof RegExp) match = keys.test(key);
  return match;
}

function findKeyJson(json, keys, min_val_len = 0) {
  let source = "";
  if (Array.isArray(json)) for (let elem of json) source = source || findKeyJson(elem, keys, min_val_len); else if (typeof json === "object") for (let elem in json) {
    let json_elem = json[elem];
    if (typeof json_elem === "string" && matchKeyJson(elem, keys)) {
      if (json_elem.length > min_val_len) return json_elem;
    } else if (Array.isArray(json_elem) && json_elem.length > 1 && matchKeyJson(elem, keys)) return json_elem; else source = source || findKeyJson(json_elem, keys, min_val_len);
  }
  return source;
}
...
function getJsonUrlText(article, callback, article_id = "", key = "", url_rest = false, url_slash = false) {
  ...
  let json_url;
  if (json_url_dom) json_url = json_url_dom.href;
  if (!json_url && article_id) json_url = window.location.origin + "/wp-json/wp/v2/posts/" + article_id;
  if (url_rest) json_url = json_url.replace("/wp-json/", "/?rest_route=/"); else if (url_slash) json_url = json_url.replace("/wp-json/", "//wp-json/");
  if (json_url) fetch(json_url).then(response => {
    if (response.ok) response.text().then(html => {
      try {
        ...
        if (json_text && json_text !== "undefined") callback(json_text, article);
      } catch (err) {
        void 0;
      }
    });
...
function getJsonUrlAdd(json_text, article, art_options = {}) {
  ...
  if (Object.keys(art_options).length) {
    if (art_options.art_type) art_type = art_options.art_type;
    if (art_options.art_class) art_attrib += ' class="' + art_options.art_class + '"';
    if (art_options.art_id) art_attrib += ' id="' + art_options.art_id + '"';
    if (art_options.art_style) art_attrib += ' style="' + art_options.art_style + '"';
    if (art_options.func_text) json_text = art_options.func_text(json_text);
  }
  let parser = new DOMParser;
  ...
  if (art_options.art_append || !article.parentNode) {
    if (!art_options.art_hold) article.innerHTML = "";
    article.appendChild(article_new);
  } else article.parentNode.replaceChild(article_new, article);
  if (func_post) func_post();
}
...
function getJsonUrl(paywall_sel, paywall_action = "", article_sel, art_options = {}, article_id = "", key = "", url_rest = false, url_slash = false) {
  ...
  if (paywall.length && article) {
    ...
    getJsonUrlText(article, (json_text, article) => {
      if (json_text && article) getJsonUrlAdd(json_text, article, art_options);
...
function randomIP(range_low = 0, range_high = 223) {
  let rndmIP = [];
  for (let n = 0; n < 4; n++) if (n === 0) rndmIP.push(range_low + randomInt(range_high - range_low + 1)); else rndmIP.push(randomInt(255) + 1);
  return rndmIP.join(".");
}
...
function ads_hide() {
  ...
  if (overlay) overlay.classList.remove("didomi-popup-open");
  ...
  if (document.querySelector('head > link[href*="/leaky-paywall"], script[src*="/leaky-paywall"], div[id^="issuem-leaky-paywall-"]')) {
    ...
    if (js_cookie && js_cookie.text.includes('"post_container":"')) {
      ...
      if (post_sel) {
        ...
        if (post) post.removeAttribute("class");
      }
    }
  }
}

/* @require-inline-end */ (function() {
  //'use strict';
  if (window.top !== window.self && !matchDomain([ "app.historytoday.com", "appan.newscientist.com" ])) return;
  ...
  if (matchDomain([ "adage.com", "autonews.com", "pionline.com" ])) {
    ...
    function adv_main() {
      if (window.adiData) window.adiData.entryTags = 0;
    }
    window.setTimeout(function() {
  ...
  window.setTimeout(function() {
    ...
    if (matchDomain("gitflic.ru")) if (window.location.pathname.startsWith("/project/magnolia1234/bpc_uploads") && document.head) {
      let sheet = document.createElement("style");
      let path_short = window.location.pathname.replace("/project/magnolia1234/bpc_uploads", "");
      if (!path_short) sheet.innerText = 'div[data-cell-type="commit"], div[data-cell-type="date"] {display: none !important;} div[data-cell-type="filename"] {flex: 0 0 100% !important;}'; else if (path_short.match(/^\/(blob|file)/)) sheet.innerText = "div.project-files-tree, div.project-files-list {flex: 0 0 50% !important; max-width: 50% !important;}";
      if (sheet.innerText) document.head.appendChild(sheet);
    }
    if (matchDomain("medium.com") || matchDomain(medium_custom_domains) || document.querySelector('head > link[href*=".medium.com/"]')) {
      ...
      if (paywall) {
        ...
        if (header) header.before(externalLink([ "freedium-mirror.cfd", "readmedium.com", "archive.today" ], "https://{domain}/{url}", url, "BPC > Try for full article text:"));
      }
      window.setTimeout(function() {
        let banner = pageContains("div > div > p", /author made this story available to/);
        if (banner.length) removeDOMElement(banner[0].parentNode.parentNode);
      }, 1e3);
    ...
    if (matchDomain("afr.com")) {
      ...
      if (article) window.setTimeout(function() {
        ...
        if (pars.length && pars.length < 5 || pagination) {
          if (pagination) removeDOMElement(pagination.parentNode); else {
          ...
          fetch(url).then(response => {
            if (response.ok) response.text().then(html => {
              if (html.includes("__REDUX_STATE__=")) try {
                ...
                if (json) {
                  ...
                  function find_item(match, p1, offset, string) {
                    ...
                    if (placeholder_id && placeholders[placeholder_id]) {
                      ...
                      if (item.data) if ([ "linkArticle", "linkExternal" ].includes(item.type)) {
                        if (item.data.text) if (item.data.url) result = '<a href="' + item.data.url + '"' + (item.data.newTab ? 'target="_blank"' : "") + ">" + item.data.text + "</a>"; else result = item.data.text;
                        ...
                        if (item.data.src) result = '<embed src="' + item.data.src + '/embed" style="width:100%" frameborder="0">';
                      } else if (item.type === "youtube") {
                        if (item.data.url) if (item.data.url.includes("watch?v=")) result = '<iframe src="' + item.data.url.replace("watch?v=", "embed/") + '" style="width: 100%; height: 400px;"></iframe>'; else result = '<a href="' + item.data.url + '" target="_blank">' + item.data.url + "</a>";
                      } else if ([ "instagram", "twitter" ].includes(item.type)) {
                        if (item.data.url) result = '<a href="' + item.data.url + '" target="_blank">' + item.data.url + "</a>";
                      } else if (item.type === "iframe") {
                        if (item.data.url) {
                          let height = 400;
                          if (item.data.url.includes("/headshots/")) height = 100;
                          result = '<iframe src="' + item.data.url + '" style="width: 100%; height: ' + height + 'px; border: none;"></iframe>';
                        }
                      } else if (![ "callout", "domainListing", "quote", "relatedStory", "video" ].includes(item.type)) void 0;
                    }
                    return result;
                  }
                  ...
                  if (json_text) {
                    ...
                    if (placeholders) json_text = json_text.replace(/<x-placeholder id="(\w+)"><\/x-placeholder>/g, find_item);
                    let parser = new DOMParser;
                    ...
                    if (article_sel.startsWith("#endOfArticle")) {
                      ...
                      if (widget) content_new.firstChild.before(widget.parentNode);
                      let author = article.querySelector('section[class$="-articleBylineToolsWrapper"]');
                      if (author) content_new.firstChild.before(author);
                    ...
                    for (let elem of key_posts) elem.href = elem.href.replace("?post=", "#");
                  }
                }
              } catch (err) {
                void 0;
    ...
    } else if (matchDomain("businessdesk.co.nz")) if (mobile) {
      func_post = function() {
        let article = document.querySelector(article_sel);
        if (article) {
      ...
      if (matchDomain(au_comm_media_domains) || document.querySelector('div#footer a[href^="https://acm.media/"]')) {
        ...
        if (mask) {
          ...
          if (subscribe_truncate) subscribe_truncate.classList.remove("subscribe-truncate");
          let subscriber_hiders = document.querySelectorAll(".subscriber-hider");
          for (let subscriber_hider of subscriber_hiders) subscriber_hider.classList.remove("subscriber-hider");
        }
        ...
        if (matchDomain(au_news_corp_domains)) {
          ...
          if (window.location.pathname.startsWith("/subscribe/") && !url.includes("/digitalprinteditions")) {
            ...
            if (window.location.pathname.includes("/video/") && document.querySelector("div.vms-premium-video")) header_nofix("div.video-hub");
        ...
        if (matchDomain("thewest.com.au") || document.querySelector('head > link[href="https://images.thewest.com.au"]')) {
            ...
            if (json_script) {
              ...
              try {
                ...
                //let json_video = json_pub.mainVideo;
                                let url = window.location.href;
                if (!url_loaded || !url.includes(url_loaded.slice(-10))) refreshCurrentTab();
                ...
                for (let par of json_content) {
                  ...
                  if (par.kind === "text") {
                  ...
                  } else if (par.kind === "embed") if (par.reference.includes("https://omny.fm/") || par.reference.includes("https://docdro.id/")) {
                    ...
                    if (par.items) {
                      ...
                      for (let item of par.items) if (item.text) {
                        ...
                        } else par_sub2 = document.createElement("span");
                    ...
                    if (par.asset.kind === "image" && par.asset.original && par.asset.original.reference) par_elem = makeFigure(par.asset.original.reference, par.asset.captionText, {
                      style: "width:100%"
                    });
                  } else if (par.kind === "inline-related") {
                    par_elem = document.createElement("p");
                    if (par.publications) for (let elem of par.publications) {
                  ...
                  if (par_elem) par_dom.appendChild(par_elem);
                }
          ...
          setInterval(function() {
            let paywall = document.querySelector(paywall_sel);
            if (paywall) thewest_main(paywall);
          }, 1e3);
      ...
      if (matchDomain("autocar.co.uk")) {
        ...
        hideDOMStyle(ads);
      } else if (matchDomain("autosport.com")) header_nofix("div.ms-article-content > p", "div.ms-piano_article-banner"); else if (matchDomain("businesspost.ie")) {
        function bpie_main() {
          if ($) {
            let article_id_dom = document.querySelector("article#article[data-uuid]");
            let article_id;
            if (article_id_dom) article_id = article_id_dom.getAttribute("data-uuid");
            if (article_id) {
          ...
          window.setTimeout(function() {
            ...
            if (article) {
              ...
              if (article_id_dom) {
                ...
                }, fix_dowjones_fetch, [ article, "div.media-layout img[src]" ]);
              }
            }
          }, 1e3);
        }
        function fnlondon_main(node) {
          window.setTimeout(function() {
            ...
            for (let elem of signin_links) {
              elem.href = "#";
 //decodeURIComponent(elem.href.split('target=')[1].split('&')[0]);
                            elem.innerText = "Open";
        ...
        func_post = function() {
          ...
          for (let elem of lazy_images) {
            ...
            if (elem.src.startsWith("data:image/") && elem.getAttribute("currentsourceurl")) elem.src = elem.getAttribute("currentsourceurl");
          }
        ...
        hideDOMStyle(ads);
      } else if (matchDomain("granta.com")) getJsonUrl("div.article-sign-up-container", "", "div.article-excerpt"); else if (matchDomain("iai.tv")) {
          ...
          if (!app) {
            ...
            if (paywall) {
              ...
              if (story) {
                ...
                if (!podcast) fetch(url_src).then(response => {
                  if (response.ok) response.text().then(html => {
            ...
            if (longread) {
              ...
              if (row) row.append(googleSearchToolLink(url.split("?")[0]));
            } else {
              ...
              article.before(div);
              let story = article.querySelector("story");
              if (story) stripAppLinks(story);
              hideDOMStyle(".bbw-embed");
            }
          }
        }
      } else if (matchDomain("literaryreview.co.uk")) getJsonUrl("p.subscribe-for-more", "", "div#_articlereview"); else if (matchDomain("lrb.co.uk")) {
        ...
        if (paywall) {
          ...
          if (paywall_inline) paywall_inline.removeAttribute("class");
          let json_script = document.querySelector("script#__NEXT_DATA__");
          if (json_script) try {
            let json = JSON.parse(json_script.text);
            if (json.props.pageProps.data.post.acf.widgets) {
              let url_next = json.props.pageProps.data.post.id;
              if (url_next && !window.location.pathname.endsWith(url_next)) refreshCurrentTab();
                ...
                if (article) {
                  ...
                  for (let par of pars) {
                    ...
                    if (par.paragraph) {
                      ...
                      if (par.text) elem.appendChild(document.createTextNode(par.text));
                    } else if ([ "image", "interactive_image" ].includes(par.acf_fc_layout)) {
                      let image_array = [];
                      if (par.image) image_array = [ par.image ]; else if (par.image_collection) image_array = par.image_collection;
                      for (let img_elem of image_array) {
                        let figure = makeFigure(img_elem.url, img_elem.caption ? img_elem.caption + " " + img_elem.description : img_elem.alt, {
                          alt: img_elem.alt,
                          style: "width: 95%;"
                        });
                    ...
                    } else if (par.acf_fc_layout === "divider") elem.appendChild(document.createElement("hr")); else if (par.acf_fc_layout === "related_articles") {
                      if (par.posts) for (let post of par.posts) if (post.link && post.title.rendered) {
                        let par_link = document.createElement("a");
                    ...
                    } else if (![ "newsletter_signup", "pull-quote" ].includes(par.acf_fc_layout)) void 0;
        ...
        if (paywall) {
          ...
          if (overlay) overlay.removeAttribute("class");
        }
        ...
        if (window.location.pathname.startsWith("/mda/")) header_nofix("div.sf-content__post", "div.sf-login-form"); else if (true) {
          ...
          if (body) {
            ...
            if (article_id) {
              ...
              getJsonUrl("div.sf-login-form", "", "div.sf-content-body__text", {
                art_append: 1
              }, article_id, "", false, true);
            }
          }
        }
        ...
        if (paywall) {
          ...
          if (content) content.removeAttribute("data-show-has-access");
        }
          ...
          if (json_script) {
            ...
            if (json) {
              ...
              if (json_text && article) {
                ...
                for (let elem of split) if (!elem.includes("(function(") && !article_text.includes(elem)) {
        ...
        func_post = function() {
          ...
          if (paywall) {
            removeDOMElement(paywall);
            let article = document.querySelector(article_sel);
            if (article) article.before(googleSearchToolLink(url));
          }
        };
        ...
        if (!matchDomain("epaper.thetimes.com")) {
          func_post = function() {
            ...
            if (article) {
              ...
              if (pars.length < 5) article.before(googleSearchToolLink(url));
              if (mobile) {
                ...
                for (let elem of inline_images) {
                  ...
                  if (parent_node.tagName === "PICTURE") parent_node = parent_node.parentNode;
                ...
                if (author) author.style = "width: 90%; margin: 20px;";
                let header = article.querySelector("div[style] > h1");
                if (header) header.parentNode.removeAttribute("style");
              }
              ...
              for (let elem of charts) {
                let div = document.createElement("div");
                if (!mobile) div.style = "margin: 0px 20%;";
                let iframe = document.createElement("iframe");
              ...
              if (read_next) {
                ...
                if (art_bottom) art_bottom.after(read_next);
              }
            }
          };
        ...
        if (preview) {
          ...
          if (premium) premium.removeAttribute("id");
        }
        ...
        if (noscroll) noscroll.classList.remove("noscroll");
        ...
        if (paywall) paywall.classList.remove("is-paywalled-article");
        ...
        if (premium) premium.classList.remove("premium", "no-entitlement");
      ...
      if (paywall) paywall.classList.remove("lab-paywall-locked");
        ...
        if (article) {
          ...
          if (json_script) try {
            ...
            if (json && json.sharedContent) {
              ...
              if (json_text) article.innerText = json_text;
            }
          } catch (err) {
            void 0;
          }
      ...
      if (paygate) paygate.removeAttribute("class"); else {
        ...
        if (article_paywall.length) {
          ...
          for (let script of scripts) if (script.text.startsWith(script_start)) if (script.text.includes('{\\"contentData\\":')) {
            json_script = script;
            break;
          }
          if (json_script) {
      ...
      func_post = function() {
        if (mobile) {
          ...
          if (article) {
            ...
            if (header) header.style.width = art_width;
            ...
            if (lang) lang.parentNode.removeAttribute("style");
            let par = article.querySelector('div[style] > div[dir="ltr"]');
            if (par) par.parentNode.style = "width: " + art_width;
          }
        }
      };
      let url = window.location.href;
      ...
      removeDOMElement(...ads);
    } else if (matchDomain("artnet.com")) if (window.location.pathname.endsWith("/amp-page")) amp_unhide_subscr_section(); else {
      let body_hidden = document.querySelector(".article-body");
      if (body_hidden) body_hidden.style = "display:block;";
        ...
        if (article) {
          ...
          if (json_script) try {
            ...
            if (json && json.props.pageProps.data.body) {
              ...
              if (!json_text.includes("<div>")) json_text = "<div>" + json_text + "</div>";
              let parser = new DOMParser;
      ...
      if (noscroll) axios_noscroll(noscroll);
      ...
      hideDOMStyle(banners);
    } else if (matchDomain("balkaninsight.com")) getJsonUrl("div.subscribeWrapper", "", "div.post_teaser", {
      art_append: 1,
      art_hold: 1
      ...
      if (paywall) {
        ...
        if (article) {
          ...
          if (article_new && article.parentNode) article.parentNode.replaceChild(article_new, article);
        }
      }
    } else if (matchDomain("barrons.com")) {
      if (window.location.pathname.startsWith("/livecoverage/")) window.setTimeout(function() {
        fix_dowjones_live();
      }, 1500); else {
        ...
        if (paywall) {
          ...
          window.setTimeout(function() {
            ...
            if (article) {
              ...
              if (article_id_dom) {
                let article_id = article_id_dom.content;
                if (article_id.includes(";id:")) article_id = article_id.split(";id:")[1].split(";")[0];
                ...
                }, fix_dowjones_fetch, [ article, 'div.media-layout img[src], div[data-block="big-top-media"] img[src]' ]);
              }
      ...
      function benz_main(node) {
        ...
        if (!window.location.pathname.startsWith("/report/")) {
          ...
          if (blurred) blurred.classList.remove("blur-lg");
        }
      }
      waitDOMElement("div.paywall-content", "DIV", benz_main, false);
    } else if (matchDomain("bhaskar.com")) if (!window.location.pathname.startsWith("/amp/")) {
      let paywall_sel = "div.paywallBlockedContent";
      let paywall = document.querySelector(paywall_sel);
      if (paywall) {
        let banner = document.querySelector(paywall_sel + " ~ div");
        if (banner) removeDOMElement(banner);
        paywall.removeAttribute("class");
      }
      let ads = document.querySelectorAll('div[style*="overflow:hidden"][style*="max-height:"], div[id^="Ad--"], article  div:empty');
      if (ads) removeDOMElement(...ads);
    } else ampToHtml(); else if (matchDomain("bizjournals.com")) {
      if (window.location.pathname.includes("/subscriber-only/")) header_nofix("div.primary"); else {
        ...
        if (paywall) {
          ...
          if (window.location.search.startsWith("?rel=gallery")) {
            if (gallery_intro) header_nofix("header", "", "BPC > go to full article", window.location.href.split("?")[0] + "?rel=plus");
            return;
          } else if (!window.location.search.startsWith("?rel=plus")) {
            window.location.href = window.location.pathname + "?rel=plus";
            return;
          }
          if (gallery_intro) header_nofix("header", "", "BPC > show intro gallery", window.location.href.split("?")[0] + "?rel=gallery");
          ...
          for (let img of header_images) {
            ...
            if (img_width_match) img.src = img.src.replace(img_width_match[1], "*" + img.width + "xx");
          }
                ...
                if (pars_index) {
                  ...
                  for (let i = pars_index; i < json.length; i++) {
                    ...
                    if (typeof par === "string" && ![ "blockquote", "default", "embed", "gallery", "horizontal_line", "image", "list", "media", "top25list" ].includes(par)) {
                      ...
                      if (style) par_new.style = style;
                      article.appendChild(par_new);
                    } else if (par.is_primary || par["primary-channel"]) break;
                  }
                }
          ...
          if (main_content && !main_content.querySelector("div")) header_nofix(main_content, "", "BPC > go to main page to pass Cloudflare check", window.location.href.split(/\/(news|stories)\//)[0]);
        }
        window.setTimeout(function() {
          ...
          if (dialog) {
            ...
            if (nuxt_inert) nuxt_inert.removeAttribute("inert");
          }
        }, 1e3);
      }
      ...
      function addLink(item, elem) {
        if (item.data && item.content) {
          if (!item.data.href.startsWith("http") && item.data.webUrl) item.data.href = item.data.webUrl;
      ...
      function addLinkValue(item_text, item_href, elem) {
        ...
        if (item_href.startsWith("http") && (!item_href.startsWith(window.location.origin) || item_href.match(/\.bloomberg\.com\/(quote|profile\/company)\//))) sub_elem.target = "_blank";
      ...
      function addMedia(par, elem) {
        if (par.subType === "video") {
          ...
          if (par.data.chart) {
            ...
            if (false && par.data.attachment && par.data.attachment.title) figure.appendChild(document.createTextNode(par.data.attachment.title));
            let img = document.createElement("img");
        ...
        } else if (par.subType === "audio") addPodcast(par.data, elem); else void 0;
      }
      ...
      function addEntity(item, elem) {
        if (item.content && item.content[0] && item.content[0].value) if (item.subType === "person") elem.appendChild(document.createTextNode(item.content[0].value)); else if (item.subType === "security") if (item.data && item.data.link && item.data.link.href) addLinkValue(item.content[0].value, item.data.link.href, elem); else elem.appendChild(document.createTextNode(item.content[0].value)); else if (item.subType === "story" && item.data && item.data.link) {
          if (item.data.link.destination && item.data.link.destination.web) addLinkValue(item.content[0].value, item.data.link.destination.web, elem); else if (item.data.link.title) elem.appendChild(document.createTextNode(item.data.link.title));
        } else void 0;
      }
      function addList(content, elem) {
        let ul = document.createElement("ul");
        ul.setAttribute("style", "list-style-type: disc; padding: 5px;");
        for (let item of content) if (item.type === "listItem") {
          let li = document.createElement("li");
          for (let list_item of item.content) if (list_item.type === "text" && list_item.value) li.appendChild(document.createTextNode(list_item.value)); else if (list_item.type === "paragraph" && list_item.content) addPar(list_item.content, li); else if (list_item.type === "link") addLink(list_item, li); else if (list_item.type === "entity" && list_item.content && list_item.content[0] && list_item.content[0].value) addEntity(list_item, li); else if (list_item.type === "div" && list_item.content) addPar(list_item.content, li); else void 0;
          ul.appendChild(li);
        }
        elem.appendChild(ul);
      }
      function addPar(content, elem) {
        for (let item of content) if (item.type === "text" && item.value) {
          let span = document.createElement("span");
          span.innerText = item.value;
          if (item.attributes) if (item.attributes.strong) span.style = "font-weight: bold"; else if (item.attributes.emphasis) span.style = "font-style: italic"; else void 0;
          elem.appendChild(span);
        } else if ([ "br", "hr" ].includes(item.type)) elem.appendChild(document.createElement(item.type)); else if (item.type === "link") addLink(item, elem); else if (item.type === "entity") addEntity(item, elem); else if ([ "paragraph", "div" ].includes(item.type)) addPar(item.content, elem); else if (item.type === "list" && item.content) addList(item.content, elem); else if (item.type === "media" && item.data) addMedia(item, elem); else if (item.type === "embed") addEmbed(item, elem); else if (item.type === "footnoteRef") {
      ...
      if (url.match(/s\/\d{4}-/)) {
        ...
        if (json_next_script) try {
          ...
          if (article_pars.length && article_pars.length < 5 && json_pars.length) {
            ...
            if (json.props.pageProps.story.aiSummary && json.props.pageProps.story.aiSummary.text && json.props.pageProps.story.aiSummary.text.length) window.setTimeout(function() {
            ...
            for (let par of json_pars) {
              ...
              if (par.type === "heading") elem.style = "font-weight: bold;";
              if ([ "heading", "paragraph" ].includes(par.type) && par.content) addPar(par.content, elem); else if (par.type.endsWith("quote") && par.content) {
                elem.appendChild(document.createElement("br"));
                for (let item of par.content) if (item.type === "paragraph" && item.content) for (let sub_item of item.content) if (sub_item.type === "text" && sub_item.value) elem.appendChild(document.createTextNode(sub_item.value + " ")); else if (sub_item.type === "entity") addEntity(sub_item, elem);
                elem.style = "font-style: italic";
              } else if (par.type === "media" && par.data) addMedia(par, elem); else if (par.type === "embed") addEmbed(par, elem); else if (par.type === "div" && par.content) {
                addPar(par.content, elem);
                elem.appendChild(document.createElement("br"));
              } else if (par.type === "list" && par.content) addList(par.content, elem); else if (par.type === "byTheNumbers" && par.content) {
                for (let item of par.content) if (item.type === "byTheNumbersItem" && item.content) for (let sub_item of item.content) if (sub_item.content) {
                  addPar(sub_item.content, elem);
                  elem.appendChild(document.createElement("br"));
                }
              } else if (![ "ad", "inline-newsletter", "inline-recirc", "tabularData" ].includes(par.type)) void 0;
              if (elem.hasChildNodes) article.appendChild(elem);
            }
            removeDOMElement(...article_pars);
            if (footnote_nr) article.appendChild(footnotes);
          }
        } catch (err) {
          void 0;
        }
          ...
          if (json_next_script) try {
            ...
            if (liveblog) {
              ...
              for (let post of posts) {
                ...
                elem.style.width = document.querySelector("main") ? document.querySelector("main").offsetWidth * .8 + "px" : "400px";
                if (post.body && post.body.content && post.body.content[0] && post.body.content[0].content) {
                  let header = post.header;
                  if (header) if (header.contributorName) {
                    ...
                    } else elem.append(document.createTextNode(header.contributorName), document.createElement("br"));
                    if (header.timestamp && header.timestamp.raw) elem.append(document.createTextNode(" - " + new Intl.DateTimeFormat("en-US", {
                      dateStyle: "long",
                      timeStyle: "short",
                      timeZone: "America/New_York"
                    }).format(new Date(header.timestamp.raw))), document.createElement("br"));
                  }
                  let pars = post.body.content[0].content;
                  for (let par of pars) if (par.type === "text" && par.value) elem.appendChild(document.createTextNode(par.value)); else if (par.type === "link") addLink(par, elem); else if (par.type.endsWith("quote") && par.content) {
                    elem.appendChild(document.createElement("br"));
                    elem.style["font-style"] = "italic";
                    addPar(par.content, elem);
                    ...
                    elem.appendChild(document.createElement("br"));
                    addPar(par.content, elem);
                  } else if (par.type === "br") elem.appendChild(document.createElement("br")); else void 0;
                  elem.appendChild(document.createElement("hr"));
                }
                if (elem.hasChildNodes()) paywall.appendChild(elem);
              }
            }
          } catch (err) {
            void 0;
        ...
        if (blur_chart) blur_chart.removeAttribute("class");
      } else if (window.location.pathname.startsWith("/profile/company/")) {
        let scripts = document.querySelectorAll('script[type="application/ld+json"]');
        let json_script;
        for (let script of scripts) if (script.innerText.includes(',"@type":"Organization",')) {
          json_script = script;
          break;
        }
        if (json_script) try {
          ...
          if (article) {
            ...
            for (let item of json_items) if (json[item]) {
              let div = document.createElement("div");
              div.style = "margin: 20px 0px;";
              if (item === "employee") {
                if (json[item].length) {
                  div.innerText = "EXECUTIVES/BOARD" + "\r\n";
                  for (let person of json[item]) if (person.name) {
                ...
                if (!json[item].startsWith("https://")) json[item] = "https://" + json[item];
      ...
      } else if (window.location.pathname.startsWith("/live/")) setInterval(function() {
        window.localStorage.clear();
      }, 15 * 60 * 1e3);
      ...
      if (article_hidden) article_hidden.removeAttribute("style");
      let ads = ".banner";
      hideDOMStyle(ads);
    } else if (matchDomain("bostonglobe.com")) {
      if (window.location.search.startsWith("?outputType=amp")) amp_unhide_subscr_section();
    } else if (matchDomain("business-standard.com")) {
      function bs_main(node) {
        removeDOMElement(node);
        let audio_tts = document.querySelector('audio[src$="-short.mp3"]');
        if (audio_tts) audio_tts.src = audio_tts.src.replace("-short", "");
        let json_script = document.querySelector("script#__NEXT_DATA__");
        if (json_script) try {
          let json = JSON.parse(json_script.text);
          if (json && json.props.pageProps.data.htmlContent) {
            ...
            if (json_text && article) {
              ...
              let article_new = doc.querySelector("div");
              if (article_new) {
                if (mobile) article_new.style = "width: 90%; margin: 20px;";
      ...
      if (!window.location.pathname.startsWith("/amp/")) {
        if (true) {
          ...
          let paywall = document.querySelector(paywall_sel);
          if (paywall) bs_main(paywall); else waitDOMElement(paywall_sel, "DIV", bs_main, false);
        }
      ...
      if (paywall) paywall.removeAttribute("hidden");
          ...
          if (json) {
            ...
            if (json_text && article) {
              ...
              for (let elem of json_pars) {
                let par;
                if (!elem.match(/[\[\]]{2}/)) if (elem.match(/img:\d+/)) {
                  if (img_main) {
                ...
                if (par) article.appendChild(par);
              }
            }
          }
        }
      ...
      setInterval(function() {
        ...
        if (url !== url_old) {
          url_old = url;
          if (url.match(/(\w+-){3,}/)) refreshCurrentTab();
        }
        ...
        if (subscr_button) subscr_button.click();
      }, 1e3);
          ...
          if (article) {
            ...
            if (intro.length < 5) {
              ...
              for (let elem of span_hidden) {
                ...
                elem.removeAttribute("class");
                if (elem.innerText) elem.innerText = elem.innerText.split("DISCLOSURES: (None)")[0];
                article.appendChild(elem);
              }
            }
          }
      ...
      if (window.localStorage.getItem("article-gate-data")) try {
      ...
      if (paywall) {
        ...
        if (article) {
          func_post = function() {
            ...
            if (div_hidden) div_hidden.removeAttribute("class");
      ...
      hideDOMStyle(ads);
    } else if (matchDomain("discovermagazine.com")) window.setTimeout(function() {
      ...
      hideDOMStyle(banners);
    }, 1e3); else if (matchDomain("dnevnik.bg")) {
      window.setTimeout(function() {
        ...
        if (paywall) {
          ...
          function addGST() {
            ...
            if (article) article.firstChild.before(googleSearchToolLink(url));
          }
          let article_lock = document.querySelector("div.article-lock");
          if (article_lock) {
            let intro = article_lock.querySelector('div[role="paragraph"]');
            let intro_start;
            if (intro) intro_start = intro.innerText.substring(0, 25);
            let scripts = document.querySelectorAll("script:not([src], [type])");
            let json_script;
            let link_script;
            let script_start = 'self.__next_f.push([1,"';
            for (let script of scripts) if (script.text.startsWith(script_start)) {
              if (!link_script && script.text.includes('significantLink\\":')) link_script = script; else if (!json_script && (script.text.includes("[storyid:") || intro_start && script.text.replace(/\\u003c/g, "<").replace(/\\u003e/g, ">").replace(/<[^<]*>\s?/g, "").replace(/\\"/g, '"').startsWith(script_start + intro_start))) json_script = script;
              if (json_script && link_script) break;
            }
            if (json_script) {
              ...
              let links;
              if (link_script) links = link_script.text.split('significantLink\\":[')[1].split('\\"],')[0].replace(/\\"/g, "").split(",");
              ...
              if (json_pars.length) {
                ...
                if (json_pars.find(x => x.includes("embed:") || x === "gallery")) addGST();
              }
              let parser = new DOMParser;
              for (let elem of json_pars) {
                let par;
                if (!elem.match(/[\[\]]{2}/)) {
                    ...
                    if (links) {
                      ...
                      if (story_id) {
                        ...
                        if (story) {
                          ...
                          if (!matchUrlDomain(window.location.hostname, story)) par.target = "_blank";
                        }
                      }
                    }
                  ...
                  if (par) article_lock.appendChild(par);
                }
              }
            }
          } else addGST();
      ...
      if (window.location.pathname.match(/^(\/amp)?\/article\//)) if (!window.location.search.startsWith("?rel=plus")) {
            ...
            if (state) try {
              ...
              if (json) {
                ...
                function find_img_url(match, p1, p2, offset, string) {
                  ...
                  if (contributorId) result = '<figure><img src="https://images2.dwell.com/photos/' + contributorId + "/" + p1 + "/original." + format + '?auto=format&q=35&w=1280"><figcaption>' + p2 + "</figcaption></figure>";
                  return result;
                }
      ...
      hideDOMStyle(ads);
    } else if (matchDomain("economictimes.com")) if (window.location.pathname.includes("/amp_")) {
      ...
      if (paywall) {
        ...
        if (content) {
          ...
          if (content_new && content.parentNode) content.parentNode.replaceChild(content_new, content);
        } else window.location.href = "https://economictimes.indiatimes.com" + window.location.pathname.replace("amp_prime", "prime");
      ...
      if (paywall || data_prime) {
        removeDOMElement(paywall);
        if (data_prime) data_prime.removeAttribute("data-prime");
        if (amphtml) amp_redirect_not_loop(amphtml); else if (window.location.pathname.startsWith("/epaper/")) window.location.href = "https://economictimes.indiatimes.com" + window.location.pathname;
      } else {
        let ads = ".adContainer";
        hideDOMStyle(ads);
      }
    }, 500); else if (matchDomain("economictimes.indiatimes.com")) {
      ...
      if (paywall) {
        ...
        if (content && full_text) if (!full_text.innerText.includes('["datawrapper-height"]')) content.innerText = breakText(full_text.innerText); else {
          let amphtml = document.querySelector('head > link[rel="amphtml"]');
          if (amphtml) amp_redirect_not_loop(amphtml);
        }
        addStyle("div.pageContent {height: auto !important;}");
        let article_wrap = document.querySelector("div.article_wrap[style]");
        if (article_wrap) article_wrap.removeAttribute("style");
      }
      if (mobile) {
        ...
        if (pageholder) {
          ...
          let header = document.querySelector("header");
          if (header) header.style = "width: 100% !important;";
      ...
      if (window.location.pathname.includes("/podcasts/")) header_nofix("section[data-body-id]", 'div[aria-labelledby="paywall-heading"]'); else if (!window.location.pathname.startsWith("/interactive/")) {
        ...
        func_post = function() {
          if (video) {
            ...
            if (video_new && video_new.parentNode) video_new.parentNode.replaceChild(video, video_new);
          }
          if (mobile) {
      ...
      hideDOMStyle(ads);
    } else if (matchDomain("epoch.org.il")) getJsonUrl("div.register-login-box", "", "div.paywall"); else if (matchDomain("espn.com")) {
      let url = window.location.href;
        ...
        if (json_script) try {
          ...
          if (json) {
            ...
            if (article) {
              ...
              let parser = new DOMParser;
              for (let pars of json) for (let par of pars) if (!par.match(/^\$\w{2}$/)) {
      ...
      if (noscroll) noscroll.removeAttribute("class");
    } else if (matchDomain("financialexpress.com")) {
      let paywall = document.querySelector("div.paywall");
      if (paywall) paywall.classList.remove("paywall");
            ...
            if (response.ok) response.json().then(json => {
              try {
                ...
                if (json_text) {
                  ...
                  if (article_new) article.after(article_new);
      ...
      if (paywall) {
        ...
        if (json_script) {
          ...
          if (json) {
            ...
            if (json_text && article) article.innerText = json_text;
          }
        }
      }
      ...
      if (content_ungated) {
        ...
        if (content_gated) {
          ...
          } else content_gated.classList.remove("content-gated");
        }
      }
      ...
      if (window.location.pathname.match(/\/ty-article(-\w+)?\//)) {
        ...
        if (!mobile) {
          ...
          if (paywall) {
            ...
            if (article_link) article_link.before(archiveLink(url));
          }
        }
      } else if (window.location.pathname.startsWith("/hblocked") && window.location.search.startsWith("?returnTo=")) header_nofix("main", "", "BPC > return to article", decodeURIComponent(window.location.search.split("?returnTo=")[1]));
      ...
      let ads = "div#ctas";
      hideDOMStyle(ads);
    } else if (matchDomain("hbr.org")) if (window.location.pathname.startsWith("/data-visuals")) {
      ...
      function hbr_main() {
        window.top.postMessage({
          type: "article-paywall:full-content"
        }, "*");
      }
      ...
      let paywall = document.querySelector(paywall_sel);
      if (true) if (paywall) hilltimes_main(paywall); else waitDOMElement(paywall_sel, "DIV", hilltimes_main, false);
      let banner = "section.hide_this_section";
      hideDOMStyle(banner);
    } else if (matchDomain("hindustantimes.com")) if (!window.location.pathname.endsWith("-amp.html")) {
      let paywall = document.querySelector(".paywall");
      if (paywall) paywall.classList.remove("paywall");
      ...
      if (trc_overlay) trc_overlay.click();
      let body_sticky = document.querySelector("body.desktopAdSticky");
      if (body_sticky) body_sticky.classList.remove("desktopAdSticky");
      ...
      hideDOMStyle(ads);
    } else if (matchDomain("historytoday.com")) if (window.location.hostname.startsWith("app.")) restorePugpigPage(); else {
      ...
      if (paywall) {
        ...
        if (locked) locked.className = "article-container";
      }
      let ads = "div.second-scroll-border";
      hideDOMStyle(ads);
    } else if (matchDomain("inc42.com")) if (window.location.pathname.endsWith("/amp/")) amp_unhide_access_hide('="status"', '="NOT status"', "div.wru-widget"); else {
      let banner = document.querySelector('div[id*="_leaderboard_"]');
      removeDOMElement(banner);
    } else if (matchDomain("indianexpress.com")) if (window.location.pathname.endsWith("/lite/")) amp_unhide_access_hide("=\"metering.result='ALLOW_ACCESS'\""); else {
      ...
      if (window.location.pathname.match(/(\/amp)?\/magazine\//)) {
        let url = window.location.href;
        if (!url.includes("/amp/")) amp_redirect("div#csc-paywall"); else amp_unhide_access_hide('="granted"', '="NOT NOT granted"', 'div[class^="wrapper_main-container_"], div.readmore__box');
      }
      ...
      let url = window.location.href;
      if (url.includes("/signin?") && url.includes("redirect_to=")) window.setTimeout(function() {
        ...
        if (dive_deeper_summary_bodies) for (let summary_body of dive_deeper_summary_bodies) if (!summary_body.querySelector("a")) {
      ...
      if (paywall && article) {
        ...
        if (fusion_script && fusion_script.text.includes("Fusion.contentCache=")) try {
              ...
              for (let par of pars) {
                ...
                if ([ "header", "text" ].includes(par.type)) {
                  if (par.content) {
                    ...
                    if (par.type === "header") elem.style = "font-weight: bold;";
                  }
                } else if (par.type === "image") {
                  if (par.url) {
                    let caption_text = par.caption;
                    if (par.credits && par.credits.affiliation && par.credits.affiliation[0] && par.credits.affiliation[0].name) caption_text += " (" + par.credits.affiliation[0].name + ")";
                  ...
                  } else elem.querySelectorAll('iframe[allow*="fullscreen"][allowfullscreen]').forEach(e => e.removeAttribute("allowfullscreen"));
                  ...
                  if (html) {
                    ...
                    if (elem_type_match) {
                      ...
                      if (embed_content) elem.appendChild(embed_content);
                    }
                  }
                } else if (![ "divider" ].includes(par.type)) void 0;
                if (elem) article.appendChild(elem);
              }
            }
          }
      ...
      if (login) {
        ...
        if (json_script) try {
          ...
          if (json && json.props.pageProps.post && json.props.pageProps.post.content) {
            ...
            if (url_next && !window.location.pathname.includes(url_next)) refreshCurrentTab();
      ...
      window.setTimeout(function() {
        ...
        if (paywall) {
          ...
          if (url_artikel) {
            ...
            if (art_hidden) art_hidden.removeAttribute("style");
            ...
            if (gallery) {
              ...
              if (json_script) try {
                ...
                for (let elem of img_pars) {
                  let figure = makeFigure(pars[elem.src], pars[elem.caption] + (elem.credit ? " " + pars[elem.credit] : ""), {
                    alt: pars[elem.alt]
                  });
            ...
            if (art_hidden) art_hidden.classList.remove("hidden");
          }
        }
      }, delay);
    } else if (matchDomain("latimes.com")) {
      let subscribers = pageContains("div.infobox > p.infobox-title", /subscribers/i);
      if (subscribers.length) removeDOMElement(subscribers[0].parentNode);
      ...
      if (paywall) {
        ...
        if (paywall_content) if (paywall_content.innerText.length) paywall_content.className = amp ? "" : "news_details_page_row2 details-story-wrapper"; else if (!amp) {
          let amphtml = document.querySelector('head > link[rel="amphtml"]');
          if (amphtml) amp_redirect_not_loop(amphtml);
        }
      }
      ...
      hideDOMStyle(ads);
    } else if (matchDomain("livemint.com")) if (window.location.pathname.includes("/amp-")) {
      ...
      if (paywall) paywall.classList.remove("paywall-notification-visible");
      window.localStorage.clear();
    } else if (matchDomain("manoramaonline.com")) if (!window.location.pathname.endsWith(".amp.html")) amp_redirect("ev-engagement"); else amp_unhide_access_hide("=\"result='ALLOW_ACCESS'\""); else if (matchDomain("marketwatch.com")) {
      if (window.location.pathname.startsWith("/livecoverage/")) window.setTimeout(function() {
        fix_dowjones_live();
      }, 1500); else {
          ...
          window.setTimeout(function() {
            ...
            if (article) {
              ...
              if (article_id_dom) {
                ...
                }, fix_dowjones_fetch, [ article, "div.img__lead img[src]" ]);
              }
            }
          }, 2e3);
        }
      ...
      if (div_hidden) div_hidden.classList.remove("tdb-block-inner");
    } else if (matchDomain("mid-day.com")) if (window.location.pathname.startsWith("/amp/")) amp_unhide_access_hide('="granted"', '="NOT granted"', '[class*="BannerAd"], div.midday-wrapper'); else {
      amp_redirect("div#myModalFullscreen");
      let read_more = document.querySelector("#read-more-my");
      if (read_more) read_more.click();
      ...
      if (paywall) natgeo_func(paywall);
      ...
      window.setTimeout(function() {
        ...
        if (overlay) overlay.classList.remove("Article__Content__Overlay--gated");
      }, 2e3);
      ...
      if (!window.location.pathname.endsWith("/amp/")) {
        ...
        if (paywall) {
          ...
          if (amphtml && !amphtml.href.includes(window.location.pathname)) {
            removeDOMElement(paywall);
            refreshCurrentTab();
          } else getJsonUrl(paywall_sel, "", "div.article-content", {
      ...
      window.setTimeout(function() {
        ...
        if (adm_button) adm_button.click();
      }, 500);
      let ads = 'div.article-ad, div[class^="PrimisVideoAdBlock_wrapper__"]';
      hideDOMStyle(ads);
    } else if (matchDomain("ndtvprofit.com")) {
      if (window.location.pathname.startsWith("/amp/")) amp_unhide_subscr_section(); else window.setTimeout(function() {
        amp_redirect('div[class^="hard-paywall"], div[class*="geotag-container_"]');
      }, 1e3);
      let ads = "div.responsive-ad";
      hideDOMStyle(ads);
    } else if (matchDomain("newcriterion.com")) getJsonUrl("div.paywall-overlay", "", "div.entry-content"); else if (matchDomain("newrepublic.com")) {
      ...
      let ads = ".ad-unit";
      hideDOMStyle(ads);
    } else if (matchDomain("newscientist.com")) if (!window.location.hostname.startsWith("appan.")) {
      ...
      hideDOMStyle(ads);
    } else restorePugpigPage(); else if (matchDomain("newsday.com")) {
      ...
      if (nd_lock) nd_lock.removeAttribute("class");
      let ads = "div.ad";
      hideDOMStyle(ads);
      ...
      if (paywall) {
        ...
        if (video) {
          let scripts = document.querySelectorAll('script[type="application/ld+json"]');
          let video_script;
          for (let script of scripts) if (script.innerText.includes('"embedUrl":"')) {
            video_script = script;
            break;
          }
          if (video_script) {
      ...
      hideDOMStyle(ads);
    } else if (matchDomain([ "nola.com", "shreveportbossieradvocate.com", "theadvocate.com" ])) if (window.location.pathname.endsWith(".amp.html")) {
      let body_hidden = document.querySelector(".site-container");
      if (body_hidden) body_hidden.setAttribute("style", "display:block;");
    } else {
      let ads = "div.tnt-ads-container, div.asset-breakout-ads";
      hideDOMStyle(ads);
    } else if (matchDomain("nv.ua")) if (!window.location.pathname.includes("/amp/")) amp_redirect('div[id^="media_paywall"]'); else {
      ...
      if (article) article.removeAttribute("class");
    } else if (matchDomain("nybooks.com")) {
      let paywall_article = document.querySelector(".paywall-article");
      if (paywall_article) paywall_article.classList.remove("paywall-article");
      ...
      if (window.location.pathname.includes("/sports/")) {
        ...
        if (paywall.length) {
          ...
          if (article) {
            ...
            if (window.navigator.userAgent.toLowerCase().includes("chrome")) header_nofix(article); else {
              let json_url_dom = document.querySelector('head > link[rel="alternate"][type="application/json"][href]');
              if (json_url_dom) {
                ...
                if (match) {
                  ...
                  }, fix_dowjones_fetch, [ article, "div.featured-image img[src]" ]);
                  article.querySelectorAll("p[class]").forEach(e => e.removeAttribute("class"));
                  if (footer) window.setTimeout(function() {
      ...
      window.setTimeout(function() {
        getArchive(url, paywall_sel + "[class]", {
          rm_attrib: "class"
        }, "article");
          ...
          if (json) {
            ...
            if (json_text && article) {
              ...
              if (par_last) {
                ...
                if (json_text.replace(/<[^<]*>/g, "").includes(par_last_str)) par_last.innerText = json_text.substring(json_text.indexOf(par_last_str)); else {
                  article.innerHTML = " ";
                  let article_new = document.createElement("p");
                  article_new.innerText = json_text;
                  article.appendChild(article_new);
      ...
      func_post = function() {
        ...
        for (let elem of hidden_images) {
          if (elem.src.startsWith("data:image/")) elem.src = elem.getAttribute("new-cursrc");
          elem.style = "width: 95%;";
        }
        ...
        for (let elem of art_bodies) {
          if (mobile) {
            ...
            if (first) {
              ...
              if (intro) intro.parentNode.parentNode.style = mobile_style;
              document.querySelectorAll("h1").forEach(e => e.style = mobile_style + " font-size: 40px;");
              first = false;
            }
          }
          let pars = elem.querySelectorAll("div");
          if (pars.length) if (pars.length < 3) header_nofix(article_sel, "", "BPC > no archive-fix");
        }
        if (!art_bodies.length) header_nofix(article_sel, "", "BPC > no archive-fix");
      };
      let url = window.location.href;
      ...
      func_post = function() {
        ...
        if (pars.length && pars.length < 5) header_nofix(article_sel, "", "BPC > no archive-fix");
      };
      let url = window.location.href;
      let article_sel = "article";
      ...
      if (pw_banner) removeDOMElement(pw_banner.parentNode);
      let overlay = document.querySelector("body.paywall-active");
      if (overlay) overlay.classList.remove("paywall-active");
      ...
      if (window.location.pathname.match(/^\/plus\/\w/)) {
        ...
        if (paywall) {
          ...
          if (fade) fade.classList.remove("fade");
        }
    ...
    } else if (matchDomain("scientificamerican.com")) setCookie("article_meter", "", "scientificamerican.com", "/", 0); else if (matchDomain("scholastic.com")) {
      ...
      if (paywall) {
        ...
        if (body_modal) body_modal.removeAttribute("class");
      }
      ...
      if (overlay) overlay.classList.remove("alert-read-limit__overlay");
            ...
            if (article) {
              ...
              if (json_script) try {
                ...
                if (json && json.props.pageProps.payload.data.article.body.json) {
                  ...
                  if (pars && pars.length) article.innerHTML = "";
                  for (let par of pars) {
                    let elem = document.createElement("p");
                    if (window.location.pathname.startsWith("/magazines/")) elem.style = "margin: 20px 0px;";
                    if ([ "p", "h3" ].includes(par.type)) for (let sub_elem of par.children) if (sub_elem.type === "text") {
                      if (sub_elem.data) elem.appendChild(document.createTextNode(sub_elem.data));
                      ...
                      if (first_child) if (first_child.type === "text") if (first_child.data) {
                        ...
                        if (sub_elem.attribs && sub_elem.attribs.href) {
                          ...
                          if (!matchUrlDomain(window.location.hostname, sub_elem.attribs.href)) a_link.target = "_blank";
                        } else if (sub_elem.type === "em") a_link.style = "font-style: italic;"; else if (sub_elem.type === "strong") a_link.style = "font-weight: bold;";
                      ...
                      if (sub_elem.attribs) {
                        ...
                        if (attribs.src) elem = makeFigure(attribs.src, attribs.title, {
                          alt: attribs.alt,
                          style: "width: 100%;"
                        }, {
                          style: "font-size: 80%;"
                    ...
                    } else void 0; else if (par.type === "div") {
                      if (par.attribs) if (par.attribs.class === "image-inline-container") {
                        ...
                        function addIframe(par_children) {
                          for (let sub_elem of par_children) {
                            if (sub_elem.children) addIframe(sub_elem.children);
                            if (sub_elem.type === "iframe") {
                              ...
                              if (attribs.src) {
                                ...
                                if (attribs.width && attribs.height) if (!attribs.width.includes("%")) {
                                  ...
                                  if (attribs.height) iframe.height = attribs.height;
                                }
                      ...
                      } else if (par.attribs.class) void 0;
                    } else if (![ "blockquote-quote", "inline-ad-slot", "inline-plus-widget", "track-viewed-percentage" ].includes(par.type)) void 0;
                    if (elem.hasChildNodes()) article.appendChild(elem);
                  }
                }
              } catch (err) {
      ...
      if (matchDomain("straitstimes.com")) {
        ...
        if (window.location.pathname.startsWith("/opinion/") || window.location.search.startsWith("?rel=plus")) {
          ...
          if (paywall) if (art_pars.length && art_pars.length < 5) window.location.href = window.location.pathname + "?rel=plus";
        }
        ...
        if (article) {
          ...
          if (par_hidden.length) {
            ...
            if (fade) fade.className = par_hidden[0].className;
          }
        }
      }
      ...
      if (limit_overlay) limit_overlay.classList.remove("limit-overlay");
    } else if (matchDomain("sloanreview.mit.edu")) {
      if (window.location.pathname.startsWith("/article/")) window.setTimeout(function() {
        let pars = document.querySelectorAll("div.article-content > p");
        if (pars.length && pars.length < 7) refreshCurrentTab();
      }, 1e3);
      ...
      removeDOMElement(...banners);
    } else if (matchDomain("spglobal.com")) setCookie("count", "", "spglobal.com", "/", 0); else if (matchDomain("standardmedia.co.ke")) {
            ...
            if (json) {
              ...
              if (json_text) {
                ...
                if (figcaption) {
                  ...
                  if (json_text.match(file_regex)) json_text = json_text.split(file_regex)[1]; else json_text = json_text.replace(/\s{2,}/g, " ").replace(figcaption.innerText.replace(/\s{2,}/g, " "), "");
                }
                let parser = new DOMParser;
      ...
      if (paywall.length) {
        ...
        if (div_hidden) div_hidden.removeAttribute("style");
        let noscroll = document.querySelector("body.overflow-hidden");
        if (noscroll) noscroll.classList.remove("overflow-hidden");
      }
      ...
      if (faded_content) faded_content.removeAttribute("class");
      ...
      removeDOMElement(...banners);
    } else if (matchDomain("swarajyamag.com")) if (!window.location.pathname.startsWith("/amp/")) window.setTimeout(function() {
      amp_redirect("div#hide-partial-content", {
        rm_attrib: "id"
      });
    }, 1e3); else {
            ...
            fetch(url_xhr).then(response => {
              if (response.ok) response.json().then(json => {
                ...
                if (json_text) {
                  ...
                  if (content) content_new.className = content.className;
      ...
      if (paywall) {
        ...
        }, "div.c-blog-post__content", art_options);
      } else {
        let img_dark = document.querySelector("div.c-hero-article__image-img.o-image");
        if (img_dark) img_dark.removeAttribute("class");
      }
      let modal = document.querySelector("div#emailsub-modal");
      removeDOMElement(modal);
      let noscroll = document.querySelector("body.modal-open");
      if (noscroll) noscroll.classList.remove("modal-open");
    } else if (matchDomain("theamericanscholar.org")) getJsonUrl("div.ssagk-form", "", "div.post_summary"); else if (matchDomain("theatlantic.com")) {
      ...
      hideDOMStyle(ads);
    } else if (matchDomain("thebulletin.org")) getJsonUrl("div.article--cropped", "", "div#body-copy", {
      art_append: 1
              ...
              if (pars) {
                ...
                if (mobile_doc) {
                  ...
                  for (let elem of pars) {
                    ...
                    } else if (elem[0] === 10) if (cards && cards[elem[1]]) {
                    ...
                    if (par_elem) mobile_doc.appendChild(par_elem);
                  }
                }
              }
            }
        ...
        if (article) {
          ...
          if (art_body) {
            ...
            if (art_img) {
              ...
              if (match) art_img_match = match[1];
            }
            let url = window.location.href.split(/[#\?]/)[0];
            fetch(url).then(response => {
              if (response.ok) response.text().then(html => {
                if (html.includes('<script>window.__remixContext.streamController.enqueue("')) try {
                  ...
                  if (json) {
                    ...
                    if (body_index) {
                      ...
                      for (let i = body_index; i < json.length; i++) {
                        let par = json[i];
                        if (typeof par === "string") if (par.startsWith("<p>")) {
                          ...
                          for (let n = i - 5; n < i; n++) {
                            let item = json[n];
                            if (typeof item === "string" && !item.startsWith("<p>") && ![ "caption", "credit_name", "file", "image" ].includes(item) && !item.match(/\d+_\d+/)) caption = caption ? caption + "\r\n" + item : item;
                          }
          ...
          if (banner) removeDOMElement(banner.parentNode);
        }
      }
      ...
      if (paywall) {
                  ...
                  if (response.ok) response.text().then(html => {
                    try {
                      if (html) {
                        ...
                        for (let par of article_new) article.appendChild(par);
        ...
        } else if (window.location.pathname.startsWith("/newsletter/")) header_nofix("section#article-body > div > p", paywall_sel);
      ...
      let fusion_script = document.querySelector("script#fusion-metadata");
      if (fusion_script && fusion_script.text.includes("Fusion.globalContent=")) try {
        ...
        if (lead_video) {
          ...
          if (json_script) {
            ...
            if (json && json.image && json.image.url) {
              let lead_fig = makeFigure(json.image.url, parseHtmlEntities(json.image.name || ""), "", {
                class: "caption"
              });
              if (mobile) lead_fig.style = "width:90%; margin: 0px 20px;";
      ...
      if (!window.location.pathname.endsWith("/amp/")) {
        if (matchDomain("www.thehindu.com")) {
          ...
          if (document.querySelector(paywall_sel) && !document.querySelector(paywall_sel + " ~ div.also-read")) amp_redirect(paywall_sel);
        } else if (matchDomain("frontline.thehindu.com")) {
          let paywall = document.querySelector("body.articlepaywall");
          if (paywall) paywall.classList.remove("articlepaywall");
          ...
          if (json && getNestedKeys(json, "props.pageProps.post.fields")) {
            ...
            window.setTimeout(function() {
              ...
              if (article) {
                article.innerHTML = "";
                article.removeAttribute("class");
                if (mobile) article.style = "margin: 0px 20px;";
                ...
                function attach_text(sub_item, elem) {
                  if (sub_item.value) {
                    ...
                    if (sub_item.marks && sub_item.marks.length) {
                      let style = "";
                      for (let mark of sub_item.marks) if (mark.type === "bold") style += "font-weight: bold;"; else if (mark.type === "italic") style += "font-style: italic;"; else if (mark.type === "underline") style += "text-decoration: underline;";
                ...
                function attach_hyperlink(sub_item, elem) {
                  if (sub_item.content && sub_item.content[0] && sub_item.content[0].value && sub_item.data && sub_item.data.uri) {
                    ...
                    if (!matchUrlDomain([ "thejuggernaut.com", "jgnt.co" ], sub_item.data.uri)) sub_elem.target = "_blank";
                ...
                function attach_paragraph(par, elem) {
                  if (par.content && par.content.length) {
                    ...
                    for (let item of par.content) if (item.nodeType === "text") attach_text(item, span_elem); else if (item.nodeType === "hyperlink") attach_hyperlink(item, span_elem); else void 0;
                    elem.appendChild(span_elem);
                  }
                }
                for (let par of pars) {
                  let elem = document.createElement("p");
                  if (par.nodeType.match(/^(paragraph|heading-\d)$/)) attach_paragraph(par, elem); else if ([ "blockquote" ].includes(par.nodeType)) {
                    if (par.content && par.content.length) for (let item of par.content) if (item.nodeType === "paragraph") {
                      elem.style = "margin: 0px 20px; font-style: italic;";
                      attach_paragraph(item, elem);
                    } else void 0;
                  } else if (par.nodeType === "hr") elem.appendChild(document.createElement("hr")); else if (par.nodeType === "embedded-asset-block") if (!par_first) {
                    if (par.data && par.data.target && par.data.target.fields) if (par.data.target.fields.file && par.data.target.fields.file.url) {
                      let figure = makeFigure(par.data.target.fields.file.url, par.data.target.fields.description);
                      elem.appendChild(figure);
                    }
                  } else par_first = false; else if (par.nodeType === "unordered-list") {
                    if (par.content && par.content.length) {
                      let ul = document.createElement("ul");
                      for (let item of par.content) if (item.nodeType === "list-item") {
                        if (item.content) for (let sub_item_par of item.content) if (sub_item_par.nodeType === "paragraph") {
                          let li = document.createElement("li");
    ...
    } else if (matchDomain("thelogic.co")) setCookie("firstarticle", "", "thelogic.co", "/", 0); else if (matchDomain("thenewatlantis.com")) {
      let article_gated = document.querySelector(".article-gated");
      if (article_gated) article_gated.classList.remove("article-gated");
      ...
      if (paywall) {
        ...
        if (fade) fade.removeAttribute("class");
        let json_script = getArticleJsonScript();
        if (json_script) {
          let json = JSON.parse(json_script.text);
          if (json) {
      ...
      removeDOMElement(...overlay);
    } else if (matchDomain("thequint.com")) window.setTimeout(function() {
      ...
      let paywall = document.querySelector("div.paywall-locker");
      if (paywall) paywall.classList.remove("paywall-locker");
      ...
      if (matchDomain("tradewindsnews.com") && window.location.pathname.startsWith("/markets/")) {
        ...
        if (overflow) overflow.removeAttribute("style");
        let blurred = document.querySelector("body > div[style]");
        if (blurred) blurred.removeAttribute("style");
      } else window.setTimeout(function() {
                ...
                if (par && par.type) {
                  ...
                  if ([ "text", "subhead" ].includes(type)) {
                    if (par.html || par.value) {
                      ...
                      if (par.value) elem.style = "font-weight: bold;";
                    }
                  } else if (type === "picture") if (img_first) img_first = false; else {
                    let caption_text = pars[par.caption];
                    if (par.credit) caption_text += " (" + pars[par.credit] + ")";
                    ...
                    elem.appendChild(sub_elem);
                  } else if (![ "ad", "ad_group", "adobetarget", "author", "break", "Emne", "job", "Location", "ma_frame", "news", "Organisasjon", "Organisation", "Organization", "promobox", "Person", "Personer", "quote", "readpeak", "Region", "Regions", "related", "Sector", "Sectors", "Selskap", "Shiptype", "Sted", "Steder", "Topic", "ukens_stilling", "video" ].includes(type)) for (let item in par) {
                    void 0;
                    void 0;
                  }
                  if (elem) article.appendChild(elem);
        ...
        let paywall = document.querySelector(paywall_sel);
        if (paywall) if (matchDomain("dn.no") && window.location.pathname.match(/^\/(d2|magasinet|smak)\//)) {
          removeDOMElement(paywall);
          let article = document.querySelector("article");
          if (article) {
            let url = window.location.href;
            ...
            if (mobile) {
              let body = document.querySelector("body");
              if (body) body.style.width = "80%";
            }
          }
        } else if (window.location.pathname.match(/^\/(e-avis|editions)/)) {
          removeDOMElement(paywall);
          header_nofix("section.wrapper");
        } else dn_main(paywall);
      }, 1e3);
              ...
              if (json) {
                ...
                for (let par of pars) {
                  ...
                  if ([ "header", "text" ].includes(par.type)) {
                    ...
                    if (par.canonical_url) if (domain.startsWith(par.canonical_website)) {
                      par_new = document.createElement("p");
                      let par_link = document.createElement("a");
                  ...
                  } else if (![ "custom_embed" ].includes(par.type)) void 0;
                  if (par_new) article.appendChild(par_new);
                }
              }
            } catch (err) {
              void 0;
      ...
      getJsonUrl("div.paywall-container", "", "div.body-copy", {
        art_class: "body-copy"
      });
      ...
      window.setTimeout(function() {
        ...
        if (paywall) {
          ...
          if (!window.location.pathname.startsWith("/amp/")) {
            ...
            if (json_script) try {
              ...
              if (json) {
                ...
                if (article) {
                  ...
                  if (content) content.parentNode.replaceChild(content_new, content); else article.appendChild(content_new);
                }
              }
            } catch (err) {
              void 0;
            ...
            if (story_hidden) story_hidden.removeAttribute("class");
          } else ampToHtml();
        }
      }, 1500);
                ...
                if (Array.isArray(elem) && elem.length) {
                  ...
                  if (typeof item === "string") {
                    if ([ "p", "h2", "em", "strong" ].includes(item)) {
                      ...
                      if (a_data.isExternal) elem_new.target = "_blank";
                      makeElem(elem.slice(2), elem_new);
                      par_elem.appendChild(elem_new);
                    } else if (item === "inline-embed" || !([ "ad", "cm-unit", "inline-newsletter", "journey-inline-newsletter", "native-ad" ].includes(item) || item.length < 30 && item.includes("inline-embed"))) if (item === "inline-embed") {
                      let img_data = elem[1];
                      if (img_data && img_data.type === "image") if (img_data.props && img_data.props.image && img_data.props.image.sources) {
                        let caption_text;
                        if (img_data.props.dangerousCaption) {
                          caption_text = img_data.props.dangerousCaption.replace(/<\/?\w+>/g, "");
                          if (img_data.props.dangerousCredit) caption_text += " " + img_data.props.dangerousCredit;
                        }
                  ...
                  } else if (Array.isArray(item)) if ([ "a", "em", "strong" ].includes(item[0])) {
                    makeElem(item, par_elem);
                    elem.shift();
                    makeElem(elem, par_elem);
                  } else void 0; else if (typeof item === "object") {
                    if (!item.class) void 0;
                    elem.shift();
                    makeElem(elem, par_elem);
                  }
                } else if (typeof elem === "string" && ![ "div" ].includes(elem)) par_elem.appendChild(document.createTextNode(elem));
              }
      ...
      hideDOMStyle(ads);
    } else if (matchDomain("warontherocks.com")) if (window.location.pathname.startsWith("/episode/")) header_nofix("div.single-content-section", 'a[href ^= "https://warontherocks.com/subscribe"]'); else {
      ...
      if (paywall) {
        ...
        if (!article) {
            ...
            if (mobile) {
              ...
              if (article) {
                ...
                if (topper) {
                  ...
                  article.querySelectorAll('div[style*="max-width:"]').forEach(e => e.style = "margin: 0px 20px; width: " + .9 * art_width);
                }
              }
            }
          };
          ...
          getArchive(url, paywall_sel, {
            rm_class: "meteredContent"
          }, "div.teaser-content", "", article_src_sel);
        }
      }
      ...
      if (window.location.pathname.startsWith("/livecoverage/")) window.setTimeout(function() {
        ...
        if (paywall) {
          ...
          if (wsj_pro) article_sel = "article";
          let article = document.querySelector(article_sel);
          if (article) {
            ...
            func_post = function() {
              ...
              if (pars.length < 5) header_nofix(article_sel, "", "BPC > no archive-fix");
              if (video) {
      ...
      if (paywall.length) {
        ...
        func_post = function() {
          ...
          if (page_hidden) page_hidden.classList.remove("hidden");
      ...
      if (!domain) domain = window.location.hostname.replace(/^(www|amp|eu)\./, "");
      ...
      if (window.location.pathname.endsWith("/restricted/") && window.location.search.startsWith("?return=")) {
        ...
        if (paywall.length) {
          removeDOMElement(...paywall);
          let article = document.querySelector("article");
          if (article) article.firstChild.before(archiveLink(url));
        }
      }
      ...
      for (let elem of subscriber_only) {
        if (elem.classList.contains("encrypted-content")) elem.innerHTML = unscramble(elem.textContent);
        elem.removeAttribute("style");
        elem.removeAttribute("class");
      }
      ...
      if (window.location.hostname.startsWith("amp.")) {
        ...
        if (subscriptions_action) subscriptions_action.removeAttribute("subscriptions-action");
        let art_cropped = document.querySelector("div.article-body.cropped");
        if (art_cropped) art_cropped.classList.remove("cropped");
        ...
        for (let elem of amp_images) {
          let img = document.createElement("img");
          img.src = elem.getAttribute("srcset").split(" ")[0], img.alt = elem.getAttribute("alt"),
          img.style = "width: 100%;";
          elem.parentNode.replaceChild(img, elem);
        }
      } else window.setTimeout(function() {
      ...
      if (window.location.pathname.endsWith("/amp/")) amp_unhide_subscr_section("div.ampWrapperInside, div#paywall"); else if (true) {
        ...
        if (paywall && article) {
          func_post = function() {
            ...
            if (iframe) article.appendChild(iframe);
          };
      ...
      if (premium) premium.removeAttribute("class");
      ...
      if (paywall) paywall.removeAttribute("class");
      hideDOMStyle("div.wkwp-paywall-block");
    }
    ads_hide();
    var leaky_paywall_unhide_disable;
    if (!leaky_paywall_unhide_disable) leaky_paywall_unhide();
  }, 1e3);
  // General Functions
  // import (see @require)
      ...
      if (json_script) try {
        ...
        if (json) {
          ...
          if (article) {
            ...
            for (let par of pars) if (par.headline && par.articleBody) {
              ...
              if (par.dateModified && par.datePublished) date.innerText = "Updated " + new Intl.DateTimeFormat("en-US", {
                dateStyle: "long",
                timeStyle: "short",
                timeZone: "America/New_York"
              }).format(new Date(par.dateModified)) + " ET / Original " + new Intl.DateTimeFormat("en-US", {
                dateStyle: "long",
                timeStyle: "short",
                timeZone: "America/New_York"
              }).format(new Date(par.datePublished)) + " ET";
    ...
    try {
      if (data) {
        ...
        if (json && json.screens && json.screens[0] && json.screens[0].frames) {
          ...
          if (intro) if (intro.tagName === "P") par_class = intro.className; else {
            let par_first = article.querySelector('p[class*="Paragraph"]');
            if (par_first) par_class = par_first.className;
          }
          if (!par_class) addStyle("div.crawler p:not([class]) {margin: 20px 0px;}");
          if (matchDomain("barrons.com")) {
            ...
            if (style_fl && style_fl.innerHTML.includes(":first-letter")) {
              ...
              for (let style of styles_fl) if (style.innerHTML.includes(":first-letter")) removeDOMElement(style);
            }
            if (document.title.startsWith("Sign in or subscribe")) if (json.screens[0].metadata) {
              let metadata = json.screens[0].metadata;
              if (metadata.title) document.title = metadata.title + " - Barrons's";
            }
          }
          let body_first = true;
          let img_lead = document.querySelector(img_lead_sel);
          let img_lead_url;
          if (img_lead) img_lead_url = img_lead.src.split("?")[0];
          article.innerHTML = "";
          function addBodyText(elem, par) {
            if (par.body && par.body.text && par.body.text !== "Visit page") if (par.body.additions) {
              ...
              if (items.length) {
                ...
                if (end_index) elem.appendChild(document.createTextNode(par.body.text.substring(end_index).replace(/\s_/g, "")));
              } else elem.innerText = par.body.text.replace(/\s_/g, "");
            } else elem.innerText = par.body.text.replace(/\s_/g, "");
          }
          for (let par of pars) {
            let elem = document.createElement("p");
            if (par_class) elem.className = par_class;
            if (par.type === "body") {
              if (par.body && par.styleID.match(/(default|bodyFrame|article-body)/)) if (body_first && intro) {
                elem = intro;
                body_first = false;
              } else addBodyText(elem, par);
            ...
            } else if (par.type === "screen") if (par.screen && par.screen.frames) {
              elem = document.createElement("p");
              ...
              sub_elem.appendChild(tr);
              elem.appendChild(sub_elem);
            } else void 0; else if (par.type === "video") {
              if (par.url) {
                let video = document.createElement("video");
                video.src = par.url;
                video.setAttribute("controls", "");
              ...
              if (par.videos && par.videos[0] && par.videos[0].url) if (window.navigator.userAgent.toLowerCase().includes("chrome")) {
                let video = document.createElement("video");
            ...
            } else if (![ "ad", "authorBios", "byline", "caption", "columnistHeader", "divider", "inlineComment", "paywall-promotion", "pullquote", "pullquotebody", "scrollinggallery", "sectionTitle", "tagCloud", "tags", "title", "verticalContainer", "webview" ].includes(par.type)) void 0;
            if (elem.hasChildNodes()) article.appendChild(elem);
          }
        }
      }
